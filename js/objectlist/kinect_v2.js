 ...
 const make_mesh = () => {
     ...
     movieMaterial = new THREE.ShaderMaterial({
			...
			vertexShader:`
				...
				void main() {				
					float mapWidth = 1024.0;
					float mapHeight = 2048.0;
					
					float depthWidth = 1024.0;
					float depthHeight = 1024.0;
					...
					}else
					{
						...
						float min_depth = 1.0;
						float max_depth = 2.5;
						float realDepth = min_depth + depth * (max_depth-min_depth);
	 ...
	 const m_depthWidth = 1024;
	 const m_depthHeight = 1024;
	 const m_xyScale = Math.tan(NUI_CAMERA_DEPTH_NOMINAL_HORIZONTAL_FOV * DegreesToRadians * 0.5) / (m_depthWidth * 0.5);
